...
For the 3D test cases, the flows around a cylinder with a square cross-section is considered. The problem configuration is illustrated in figure \ref{fig:3D}. The boundary condition is defined to be 0 for both Dirichlet and Neumann, except for the inflow and the outflow. The characteristic velocity is \(U(t) = \frac{4U(0, H/2, H/2, t)}{9}\), and the Reynolds number is defined by \(Re = \frac{UD}{\nu}\). The drag and lift forces are
\begin{align*}
...
To solve this problem we used only the aSIMPLE preconditioner with \Delta \(t\)=0.05 in a time interval 0<=\(t\)<=8. 

We show for each time step the time taken to build the preconditioner, the time taken to solve the problem and both the drag and lift coefficients.

The parallel implementation used 5 physiscal cores, and the test is executed for both a coarse and a more fine mesh.

/*PICTURES OF RESULTS COARSE/FINE FOR BOTH PRECONDITIONER AND SOLVER (IN MILLIS) */
/*PICTURES OF DRAG AND LIFT OVER TIME*/

As we can see the time taken to construct the preconditioner stays the same, while the time taken to solve the problem can vary a lot, also becoming much smaller then the preconditioner. This occurs because the problem is steady and the solution over time stabilizes and less iterations are required to solve the problem.

No major differences are found elsewhere, the drag and lift coefficients over time remain pretty much similar indipendently from the granularity of the problem.
